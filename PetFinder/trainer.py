...
class MyEngine(Engine):
    ...
    def train(engine, mini_batch):
        ...
        image, feature, pawpularity = image.to(engine.device).float(), feature.to(engine.device).float(), pawpularity.to(engine.device).float()

        # Teke feed-forward
        y_hat = engine.model(image, feature)

        loss = engine.crit(y_hat, pawpularity)
        loss.backward()
    ...
    def attach(train_engine, validation_engine, verbose=VERBOSE_BATCH_WISE):
        ...
        if verbose >= VERBOSE_EPOCH_WISE:
            ...
            def print_train_logs(engine):
                print(f'Epoch {engine.state.epoch} - |param|={engine.state.metrics["|param|"]:.4f}  loss={engine.state.metrics["loss"]:.4f}  accuracy={engine.state.metrics["accuracy"]:.4f}')
